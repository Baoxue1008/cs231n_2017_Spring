...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    first_fc_res = np.dot(X, W1) + b1
    relu_res = first_fc_res
    relu_res[relu_res < 0] = 0
    scores = np.dot(relu_res, W2) + b2
    ...
    correct_class_scores = shift_scores[range(shift_scores.shape[0]), y]
    ...
    dSoft[range(dSoft.shape[0]), y] -= 1
    dW2 = np.dot(relu_res.T, dSoft)
    dW2 /= N
    dW2 += 2 * W2 * reg
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      rand_index = np.random.permutation(num_train)[0 : batch_size]
